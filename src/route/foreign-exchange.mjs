...
export const route = async ({ message, say }) => {
  ...
  const currency = message.text.replace(/^fx:/i, '').replace('刀', '美元');
...
number: xxx;
  ...
  const currencyConverter = new CC({ from: code, to: 'CNY', amount  });
  const cny = await currencyConverter.convert();
  const rates = cny / amount;
  await say({ text: `${cny}元，汇率：1${code}=${rates}cny`, thread_ts });
};
